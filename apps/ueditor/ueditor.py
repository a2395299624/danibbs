...
ueditor_bp = Blueprint('ueditor', __name__, url_prefix='/ueditor')
...
@ueditor_bp.before_app_first_request
...
@ueditor_bp.route('/upload/', methods=['GET', 'POST'])
def upload():
    action = request.args.get('action')
    result = {}
    if action == 'config':
        config_path = os.path.join(ueditor_bp.static_folder or app.static_folder, 'ueditor', 'config.json')
...
@ueditor_bp.route('/files/<filename>/')
def files(filename):
